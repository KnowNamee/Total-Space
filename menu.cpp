...
void MainMenu::Draw() {
  ...
  txt_total_space_->setScale(10);
  txt_total_space_->setPos(cp - QPoint(390, 400));

  btn_exit_->setScale(5);
  btn_exit_->setPos(cp - QPoint(50, 0));

  btn_new_game_->setScale(5);
  btn_new_game_->setPos(cp - QPoint(50, 20));
}
...
void PauseMenu::Draw() {
  ...
  btn_back_->setScale(5);
  btn_exit_->setScale(5);
  btn_back_->setPos(cp - QPoint(30, 0));
  btn_exit_->setPos(cp - QPoint(30, 40));
}
...
void PlanetMenu::Draw() {
  ...
  radius_ = -(p->Radius() *
              1.2);  // TODO определить зависимость множителя от радиуса планеты
